...
object MiLinkServiceHook : HookContext() {
    ...
    private fun registerStatusReceiver(ctx: Context?) {
        ...
        val filter = IntentFilter().apply {
            ...
            addAction(HyperPodsAction.ACTION_CUSTOM_BUTTON_FUNCTION_CHANGED)
            addAction(HyperPodsAction.ACTION_CUSTOM_BUTTON_POSITION_CHANGED)
        }
        val receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                when (intent?.action) {
                    ...
                    HyperPodsAction.ACTION_CUSTOM_BUTTON_FUNCTION_CHANGED -> {
                        refreshCustomButtonFunction(intent)
                        saveState(context)
                        // 立即刷新面板中当前选择位置的自定义控件。
                        notifyFindRingChanged()
                    }
                    HyperPodsAction.ACTION_CUSTOM_BUTTON_POSITION_CHANGED -> {
    ...
    private fun requestBluetoothStatus(reason: String, allowReconnect: Boolean = false) {
        ...
        ctx.sendBroadcast(Intent(HyperPodsAction.ACTION_REFRESH_STATUS).apply {
            putExtra(HyperPodsAction.EXTRA_ALLOW_RFCOMM_RECONNECT, allowReconnect)
