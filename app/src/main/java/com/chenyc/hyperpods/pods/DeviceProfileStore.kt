...
object DeviceProfileStore {
    private const val TAG = "HyperPods-ProfileStore"
    ...
    fun profileMode(prefs: SharedPreferences): ProfileMode =
        ProfileMode.fromPreference(prefs.getString(HyperPodsPrefsKey.PROFILE_MODE, null))

    fun setProfileMode(prefs: SharedPreferences, mode: ProfileMode) {
        prefs.edit().putString(HyperPodsPrefsKey.PROFILE_MODE, mode.preferenceValue).apply()
    }

    /** 手动模式下选中的白名单 productId；未选则为 null。 */
    fun selectedModelId(prefs: SharedPreferences): String? =
        prefs.getString(HyperPodsPrefsKey.SELECTED_MODEL_ID, null)?.takeIf { it.isNotBlank() }

    fun setSelectedModelId(prefs: SharedPreferences, modelId: String) {
        prefs.edit().putString(HyperPodsPrefsKey.SELECTED_MODEL_ID, modelId).apply()
    }
