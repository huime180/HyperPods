...
import com.chenyc.hyperpods.utils.MediaControl
...
import com.chenyc.hyperpods.utils.miuiStrongToast.MiuiStrongToastUtil.cancelPodsNotificationByMiuiBt
...
object RfcommController {
    ...
    private fun changeUIAncStatus(status: Int) {
        ...
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_ANC_CHANGED) {
    ...
    private fun changeUIBatteryStatus(status: BatteryParams) {
        ...
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_BATTERY_CHANGED) {
            putBatteryStatus(status)
        }
    }

    private fun changeUIGameModeStatus(enabled: Boolean) {
        Intent(HyperPodsAction.ACTION_PODS_GAME_MODE_CHANGED).apply {
            this.putExtra("enabled", enabled)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_GAME_MODE_CHANGED) {
    ...
    private fun changeUIEqStatus() {
        Intent(HyperPodsAction.ACTION_PODS_EQ_PRESET_CHANGED).apply {
            addEqExtras(this)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext?.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_EQ_PRESET_CHANGED) {
            addEqExtras(this)
        }
    }
    ...
    private fun changeUISpatialAudioStatus(mode: Int) {
        ...
        Intent(HyperPodsAction.ACTION_PODS_SPATIAL_AUDIO_CHANGED).apply {
            this.putExtra("mode", normalizedMode)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_SPATIAL_AUDIO_CHANGED) {
            putExtra("mode", normalizedMode)
        }
    }

    private fun changeUISpatialSoundStatus(enabled: Boolean) {
        Intent(HyperPodsAction.ACTION_PODS_SPATIAL_SOUND_CHANGED).apply {
            this.putExtra("enabled", enabled)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_SPATIAL_SOUND_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUINoiseLevelStatus(level: Int) {
        Intent(HyperPodsAction.ACTION_PODS_NOISE_LEVEL_CHANGED).apply {
            this.putExtra("level", level)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_NOISE_LEVEL_CHANGED) {
            putExtra("level", level)
        }
    }

    private fun changeUISmartAncLevel(level: Int) {
        Intent(HyperPodsAction.ACTION_PODS_SMART_ANC_LEVEL_CHANGED).apply {
            this.putExtra("level", level)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
    ...
    private fun changeUIAutoPlayPauseStatus(enabled: Boolean) {
        Intent(HyperPodsAction.ACTION_PODS_AUTO_PLAY_PAUSE_CHANGED).apply {
            this.putExtra("enabled", enabled)
            this.`package` = BuildConfig.APPLICATION_ID
            this.addFlags(Intent.FLAG_RECEIVER_FOREGROUND)
            mContext!!.sendBroadcast(this)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_AUTO_PLAY_PAUSE_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUIDualDeviceStatus(enabled: Boolean) {
        Intent(HyperPodsAction.ACTION_PODS_DUAL_DEVICE_CHANGED).apply {
            this.putExtra("enabled", enabled)
        ...
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_DUAL_DEVICE_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUIConnectedDevicesStatus(devices: List<ConnectedDevice>) {
        Intent(HyperPodsAction.ACTION_PODS_CONNECTED_DEVICES_CHANGED).apply {
    ...
    private fun sendBtLogBroadcast(isSend: Boolean, packet: ByteArray, label: String?) {
        ...
        Intent(HyperPodsAction.ACTION_BT_LOG_ENTRY).apply {
            setPackage(BuildConfig.APPLICATION_ID)
            putExtra(HyperPodsAction.EXTRA_BT_LOG_IS_SEND, isSend)
            putExtra(HyperPodsAction.EXTRA_BT_LOG_HEX, packet.toHexString(HexFormat.UpperCase))
            if (label != null) putExtra(HyperPodsAction.EXTRA_BT_LOG_LABEL, label)
    ...
    private fun loadNotificationSettings(context: Context): NotificationSettings {
        ...
        val cachedSettings = context.getSharedPreferences(
            HyperPodsPrefsKey.NOTIFICATION_SETTINGS_CACHE_PREFS_NAME,
            Context.MODE_PRIVATE
        ).let { NotificationSettings.fromPrefsOrNull(it) }
        if (
            remoteSettings.updatedAt == 0L &&
            mPrefs.contains(HyperPodsPrefsKey.SHOW_CONNECTION_NOTIFICATION) &&
    ...
    fun handleUIEvent(intent: Intent, receiverContext: Context? = null) {
        when (intent.action) {
            HyperPodsAction.ACTION_PODS_UI_INIT -> {
                ...
                sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_CONNECTED) {
            ...
            HyperPodsAction.ACTION_REFRESH_STATUS -> {
                val allowReconnect = intent.getBooleanExtra(
                    HyperPodsAction.EXTRA_ALLOW_RFCOMM_RECONNECT,
                    false
                )
                queryStatus(allowReconnect)
            }
            HyperPodsAction.ACTION_GAME_MODE_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
                setGameMode(enabled)
            }
            HyperPodsAction.ACTION_EQ_PRESET_SET -> {
                setEqPreset(intent.getIntExtra("id", -1))
            }
            HyperPodsAction.ACTION_EQ_PRESET_SAVE -> {
                saveEqPresetFromIntent(intent)
            }
            HyperPodsAction.ACTION_EQ_PRESET_DELETE -> {
                val entry = DeviceProfileStore.parseEqEntries(
                    intent.getStringExtra(HyperPodsAction.EXTRA_EQ_ENTRIES_JSON)
                ).firstOrNull()
                if (entry != null) deleteEqPreset(entry)
            }
            HyperPodsAction.ACTION_SPATIAL_AUDIO_SET -> {
                val mode = intent.getIntExtra("mode", SpatialAudioMode.OFF)
                setSpatialAudioMode(mode)
            }
            HyperPodsAction.ACTION_SPATIAL_SOUND_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
                setSpatialSound(enabled)
            }
            HyperPodsAction.ACTION_NOISE_LEVEL_SET -> {
                val level = intent.getIntExtra("level", NoiseLevel.DEEP)
                setNoiseLevel(level)
            }
            HyperPodsAction.ACTION_AUTO_PLAY_PAUSE_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
                setAutoPlayPause(enabled)
            }
            HyperPodsAction.ACTION_DUAL_DEVICE_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
                setDualDevice(enabled)
            }
            HyperPodsAction.ACTION_CYCLE_ANC -> {
                cycleAnc()
            }
            HyperPodsAction.ACTION_NOTIFICATION_SETTINGS_CHANGED -> {
                syncNotificationSettings(receiverContext ?: mContext, intent, refreshNotification = true)
            }
            HyperPodsAction.ACTION_ACTIVE_PROFILE_CHANGED -> {
                val jsonStr = intent.getStringExtra(HyperPodsAction.EXTRA_PROFILE_JSON)
    ...
    fun handleBatteryChanged(result: BatteryParser.BatteryResult, preserveMissing: Boolean = false) {
        ...
        // 导致连接后自定义音效列表为空（参考 HyperPodsManager 的 _wasConnected 补查逻辑）
    ...
    private fun showConnectionPopup(context: Context, batteryParams: BatteryParams) {
        try {
            Intent().apply {
                setClassName(BuildConfig.APPLICATION_ID, "com.chenyc.hyperpods.ConnectionPopupActivity")
    ...
    fun connectPod(context: Context, device: BluetoothDevice, prefs: SharedPreferences) {
        ...
        context.registerReceiver(broadcastReceiver, IntentFilter().apply {
            ...
            this.addAction(HyperPodsAction.ACTION_REFRESH_STATUS)
            this.addAction(HyperPodsAction.ACTION_GAME_MODE_SET)
            this.addAction(HyperPodsAction.ACTION_EQ_PRESET_SET)
            this.addAction(HyperPodsAction.ACTION_EQ_PRESET_SAVE)
            this.addAction(HyperPodsAction.ACTION_EQ_PRESET_DELETE)
            this.addAction(HyperPodsAction.ACTION_SPATIAL_AUDIO_SET)
            this.addAction(HyperPodsAction.ACTION_SPATIAL_SOUND_SET)
            this.addAction(HyperPodsAction.ACTION_NOISE_LEVEL_SET)
            this.addAction(HyperPodsAction.ACTION_AUTO_PLAY_PAUSE_SET)
            this.addAction(HyperPodsAction.ACTION_DUAL_DEVICE_SET)
            this.addAction(HyperPodsAction.ACTION_CYCLE_ANC)
            this.addAction(HyperPodsAction.ACTION_NOTIFICATION_SETTINGS_CHANGED)
            this.addAction(HyperPodsAction.ACTION_ACTIVE_PROFILE_CHANGED)
    ...
    private fun broadcastResolvedProfile(profile: DeviceProfile) {
        val context = mContext ?: return
        Intent(HyperPodsAction.ACTION_PODS_PROFILE_CHANGED).apply {
            setPackage(BuildConfig.APPLICATION_ID)
