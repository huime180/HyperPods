...
object RfcommController {
    ...
    private fun changeUIAncStatus(status: Int) {
        ...
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_ANC_CHANGED) {
    ...
    private fun changeUIBatteryStatus(status: BatteryParams) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_BATTERY_CHANGED) {
            if (::mDevice.isInitialized) this.putExtra("address", mDevice.address)
            this.putExtra("status", status)
    ...
    private fun changeUIWearStatus(status: WearStatus) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_WEAR_STATUS_CHANGED) {
            if (::mDevice.isInitialized) this.putExtra("address", mDevice.address)
            putWearStatusExtras(status)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_WEAR_STATUS_CHANGED) {
            putWearStatusExtras(status)
        }
    }

    private fun changeUIGameModeStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_GAME_MODE_CHANGED) {
    ...
    private fun changeUITransparencyVocalEnhancementStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_TRANSPARENCY_VOCAL_ENHANCEMENT_CHANGED) {
            this.putExtra("enabled", enabled)
        }
        sendExternalPodsStatusBroadcast(HyperPodsAction.ACTION_PODS_TRANSPARENCY_VOCAL_ENHANCEMENT_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUISpatialAudioStatus(mode: Int) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_SPATIAL_AUDIO_CHANGED) {
            this.putExtra("mode", mode)
        }
    }

    private fun changeUIEqPreset(presetId: Int) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_EQ_PRESET_CHANGED) {
    ...
    private fun changeUISmartAncLevel(ordinal: Int) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_SMART_ANC_LEVEL_CHANGED) {
            this.putExtra("ordinal", ordinal)
        }
    }

    private fun changeUIDualDeviceConnectionStatus(enabled: Boolean) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_DUAL_DEVICE_CONNECTION_CHANGED) {
            this.putExtra("enabled", enabled)
        }
    }
        ...
        when (intent.action) {
            HyperPodsAction.ACTION_PODS_UI_INIT -> {
                ...
                if (::mDevice.isInitialized && isConnected) {
                    sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_CONNECTED) {
            ...
            HyperPodsAction.ACTION_PODS_UI_CLOSED -> {
                appUiActive = false
            ...
            HyperPodsAction.ACTION_AUTO_GAME_MODE_CHANGED -> {
            ...
            HyperPodsAction.ACTION_TRANSPARENCY_VOCAL_ENHANCEMENT_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
            ...
            HyperPodsAction.ACTION_EQ_PRESET_DELETE -> {
                val entries = runCatching {
                    Json.decodeFromString(
                        ...
                        intent.getStringExtra(HyperPodsAction.EXTRA_EQ_ENTRIES_JSON).orEmpty(),
                    )
                }.getOrDefault(emptyList())
                entries.firstOrNull()?.let(::deleteEqPreset)
            }
            HyperPodsAction.ACTION_DUAL_DEVICE_CONNECTION_SET -> {
                val enabled = intent.getBooleanExtra("enabled", false)
            ...
            HyperPodsAction.ACTION_RFCOMM_LOG_CONNECT -> {
                if (!RfcommLog.isEnabled()) {
                    RfcommLog.setEnabled(true, mContext)
                }
            }
            HyperPodsAction.ACTION_RFCOMM_LOG_DISCONNECT -> {
                RfcommLog.setEnabled(false)
            }
            HyperPodsAction.ACTION_RFCOMM_LOG_CLEAR -> {
                RfcommLog.clear()
            }
            HyperPodsAction.ACTION_RFCOMM_DEBUG_SEND -> {
    ...
    private fun changeUIConnectionState(state: String) {
        sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_CONNECTION_STATE_CHANGED) {
    ...
    fun connectPod(context: Context, device: BluetoothDevice, prefs: SharedPreferences, appRequested: Boolean = false) {
        ...
        if (!receiverRegistered) {
            context.registerReceiver(broadcastReceiver, IntentFilter().apply {
                ...
                this.addAction(HyperPodsAction.ACTION_PODS_UI_CLOSED)
                this.addAction(HyperPodsAction.ACTION_REFRESH_STATUS)
                this.addAction(HyperPodsAction.ACTION_GAME_MODE_SET)
                this.addAction(HyperPodsAction.ACTION_AUTO_GAME_MODE_CHANGED)
                this.addAction(HyperPodsAction.ACTION_TRANSPARENCY_VOCAL_ENHANCEMENT_SET)
                ...
                this.addAction(HyperPodsAction.ACTION_DUAL_DEVICE_CONNECTION_SET)
                this.addAction(HyperPodsAction.ACTION_CYCLE_ANC)
                this.addAction(HyperPodsAction.ACTION_CONFIG_CHANGED)
                this.addAction(HyperPodsAction.ACTION_RFCOMM_LOG_CONNECT)
                this.addAction(HyperPodsAction.ACTION_RFCOMM_LOG_DISCONNECT)
                this.addAction(HyperPodsAction.ACTION_RFCOMM_LOG_CLEAR)
                this.addAction(HyperPodsAction.ACTION_RFCOMM_DEBUG_SEND)
    ...
    fun disconnectedPod(context: Context, device: BluetoothDevice) {
        ...
        mContext?.let {
            ...
            sendAppStatusBroadcast(HyperPodsAction.ACTION_PODS_DISCONNECTED) {
